...
import './top-favorited-users.css';
...
export default class TopFavoritedUsers extends React.Component {
  renderTweets = (tweet) => {
    return(
  ...
  renderUser(userData, user) {
    ...
    const selectedUser = this.state.selected || _.keys(this.props.topFavoritedUsers)[0];
      ...
      <div className={className} key={user} >
        <Row>
          ...
          <Col className="buttons" sm={5}>
            <button className="btnTweets buttonsTweetFollowers pr-1"> {convertNumbers(userData.user_statuses_count)} tweets </button>
            <button className="btnFollowers buttonsTweetFollowers pr-1"> {convertNumbers(userData.user_followers_count)} followers </button>
            <button className="btnFavorited"> {convertNumbers(userData.favorite_count)} favorited </button>
          </Col>
        </Row>
      </div>
    );
  ...
  render() {
    const selected = this.state.selected || _.keys(this.props.topFavoritedUsers)[0];
    const { topFavoritedUsers } = this.props;
    const topFavoritedUsersList = _.map(topFavoritedUsers, this.renderUser);
    const tweetsList = _.get(topFavoritedUsers, [selected, "top_tweets"], []).map(this.renderTweets);

    return (
      <section className="topFavoritedUsersContainer">
        <div className="section-title">
          <SectionTitle value="TOP FAVORITED USERS" />
        </div>
          ...
          <Row>
            <Col className="users-list" sm={6}>
              <div className="users-container">
                {
                  !_.isEmpty(topFavoritedUsersList) &&
                  <Slider
                    infinite={false}
                    slidesToShow={4}
                    slidesToScroll={1}
